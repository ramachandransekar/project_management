...
const ProgressTracking = () => {
  ...
  const fetchProjects = async () => {
    try {
      ...
      const response = await fetch('https://project-management-1-kkb0.onrender.com/api/progress/projects', {
        headers: {
  ...
  const fetchProjectProgress = async (projectId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://project-management-1-kkb0.onrender.com/api/progress/project/${projectId}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const fetchTeamLeaderboard = async (projectId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://project-management-1-kkb0.onrender.com/api/progress/project/${projectId}/leaderboard`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
